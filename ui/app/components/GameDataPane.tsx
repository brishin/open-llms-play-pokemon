...
export function GameDataPane({
	...
	const gameStateFiles = artifacts
		.filter(
			(artifact) =>
				artifact.path?.startsWith("game_state_step_") &&
	...
	if (gameStateFiles.length === 0) {
		return (
			<GameDataPaneContainer>
				<div className="mt-[0.5lh]  text-muted">
		...
		return (
			<GameDataPaneContainer>
				<div className="mt-[0.5lh]  text-muted">Loading...</div>
			</GameDataPaneContainer>
		);
	} else if (!fetcher.data) {
		return (
			<GameDataPaneContainer>
				<div className="mt-[0.5lh]  text-muted">Failed to load game data.</div>
			</GameDataPaneContainer>
		);
	}

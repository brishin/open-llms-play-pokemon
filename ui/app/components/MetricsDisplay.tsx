...
export function MetricsDisplay({ run }: MetricsDisplayProps) {
  ...
  return (
    <BoxContainer shear="top" className="p-4">
      <span variant-="background">Game Metrics</span>
      
      <div className="mt-[1lh] grid grid-cols-2 gap-[4ch]">
        <div className="space-y-[0.5lh]">
          ...
          <div className="space-y-[0.25lh] text-sm">
            ...
            <div className="flex justify-between">
              <span>In Battle:</span>
              <span className="font-mono">{latestMetrics.is_in_battle?.value ? 'Yes' : 'No'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-[0.5lh]">
          ...
          <div className="space-y-[0.25lh] text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono">{metrics.length > 0 ? Math.max(...metrics.map(m => m.step)) : 0}</span>
            </div>
            <div className="flex justify-between">
              <span>Duration:</span>
            ...
            <table className="w-full">
              ...
              <tbody>
                {metrics
                  ...
                  .slice(0, 10)
                  .map((metric) => {
                    const sameStepMetrics = metrics.filter(m => m.step === metric.step);
                    const badges = sameStepMetrics.find(m => m.key === 'badges')?.value ?? 0;
                    const party = sameStepMetrics.find(m => m.key === 'party_count')?.value ?? 0;
                    const map = sameStepMetrics.find(m => m.key === 'current_map')?.value ?? 0;
                    const battle = sameStepMetrics.find(m => m.key === 'is_in_battle')?.value ?? 0;
                    ...
                    return (
                      <tr key={metric.step} className="border-b">
                        ...
                        <td className="p-[0.25ch] font-mono">{battle ? 'Yes' : 'No'}</td>
                      </tr>
                    );
                  })}
              </tbody>

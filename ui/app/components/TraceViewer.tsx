...
function PredictionStepCard({ span, stepNumber }: { span: MLFlowSpan; stepNumber: number }) {
  ...
  return (
    <div className="border border-surface0 rounded p-[1ch] mb-[0.5lh]">
      ...
      <div className="mb-[0.5lh]">
        <div className="text-sm font-medium text-subtext1 mb-[0.25lh]">Thought:</div>
        <div className="text-text">{prediction.next_thought}</div>
      </div>
      
      <div>
        <div className="text-sm font-medium text-subtext1 mb-[0.25lh]">Action:</div>
...
export default function TraceViewer({ runId, experimentId }: TraceViewerProps) {
  const [spans, setSpans] = useState<MLFlowSpan[]>([]);
  ...
  useEffect(() => {
    const loadTraces = async () => {
      try {
        ...
        const traceArtifact = await mlflowClient.getTraceForRun(experimentId, runId);
        
        if (traceArtifact?.spans) {
          const predictionSpans = filterPredictionSpans(traceArtifact.spans);
          setSpans(predictionSpans);
        } else {
          setSpans([]);
        }
      } catch (err) {
  ...
  return (
    <BoxContainer title="DSPy Predictions">
      <div className="max-h-[20lh] overflow-y-auto">
        ...
        {error && (
          <div className="text-center text-red py-[2lh]">
            Error: {error}
          </div>
        )}
